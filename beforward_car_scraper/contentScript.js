...
function normalizeEngine(engineStr) {
  ...
  const match = engineStr.replace(/,/g, '').match(/\d+/);
...
function scrapeCarData() {
  ...
  const modelBody = breadcrumb.carModel && modelCode && breadcrumb.bodyType
  ...
  chrome.runtime.sendMessage({ type: "BEFORWARD_CAR_DATA", payload: data });
}
